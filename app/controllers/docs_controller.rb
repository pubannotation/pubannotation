...
class DocsController < ApplicationController
	...
	def index
		...
		# Build ID-based lookup to avoid misalignment when records are missing from DB
		if use_elasticsearch
			@results_by_doc_id = @search_results.results.index_by { |r| r.id.to_i }
		end

		@search_keywords = params[:keywords]&.strip&.downcase if use_elasticsearch

		respond_to do |format|
			format.html do
				# HTML snippets are generated by doc_snippet helper using @search_keywords
				# directly against the original body, so no need to replace doc.body here.
			end
			format.json do
				docs = if use_elasticsearch
								 @docs.map { |doc| set_highlight_text doc.to_list_hash, @results_by_doc_id[doc.id]&.highlight&.body }
							 else
			...
			format.tsv do
				docs = if use_elasticsearch
								 @docs.map { |doc| set_highlight_text doc.to_list_hash, @results_by_doc_id[doc.id]&.highlight&.body&.first }
							 else
								 @docs.map(&:to_list_hash)
							 end
