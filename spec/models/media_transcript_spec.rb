...
RSpec.describe MediaTranscript, type: :model do
  ...
  describe '#speech_segments' do
    it 'includes all segments when every segment is spoken text' do
    ...
    it 'excludes non-speech segments' do
      media_transcript = build(:media_transcript, segments: [
    ...
    it 'keeps only the spoken segments alongside non-speech segments' do
      media_transcript = build(:media_transcript, segments: [
        { 'text' => '(upbeat music)', 'start_ms' => 0, 'end_ms' => 3000 },
    ...
    it 'excludes segments with blank text' do
      media_transcript = build(:media_transcript, segments: [
  ...
  describe '#segments_with_char_offsets' do
    it 'sets each speech segment\'s offset into the text #speech_text would build' do
      media_transcript = build(:media_transcript, segments: [
        { 'text' => 'Hello', 'start_ms' => 0, 'end_ms' => 300 },
        { 'text' => 'world', 'start_ms' => 300, 'end_ms' => 600 }
      ])

      expect(media_transcript.segments_with_char_offsets).to eq([
        { 'text' => 'Hello', 'start_ms' => 0, 'end_ms' => 300, 'char_begin' => 0, 'char_end' => 5 },
        { 'text' => 'world', 'start_ms' => 300, 'end_ms' => 600, 'char_begin' => 6, 'char_end' => 11 }
      ])
    end

    it 'sets nil offsets on non-speech segments, without advancing the position for later ones' do
      media_transcript = build(:media_transcript, segments: [
        { 'text' => '(upbeat music)', 'start_ms' => 0, 'end_ms' => 3000 },
      ...
      expect(media_transcript.segments_with_char_offsets).to eq([
        { 'text' => '(upbeat music)', 'start_ms' => 0, 'end_ms' => 3000, 'char_begin' => nil, 'char_end' => nil },
        { 'text' => 'Welcome.', 'start_ms' => 3000, 'end_ms' => 6000, 'char_begin' => 0, 'char_end' => 8 }
      ])
    end
    ...
    it 'passes malformed segments through unchanged' do
      ...
      expect(media_transcript.segments_with_char_offsets).to eq(['not a hash'])
    end
  end

  describe 'building a new record with segments' do
    it 'derives text and stores char offsets on the segments, matching MediaTextGenerationService' do
      medium = create(:medium, media_type: :audio, content_type: 'audio/mpeg')
    ...
    it 'derives text from speech segments only, while still setting offsets for non-speech ones to nil' do
      medium = create(:medium, media_type: :audio, content_type: 'audio/mpeg')
      ...
      expect(media_transcript.segments).to eq([
        { 'text' => '(upbeat music)', 'start_ms' => 0, 'end_ms' => 3000, 'char_begin' => nil, 'char_end' => nil },
        { 'text' => 'Welcome to the conference.', 'start_ms' => 3000, 'end_ms' => 6000, 'char_begin' => 0, 'char_end' => 26 }
      ])
    end
